...
export default function EditQuizPage() {
  ...
  useEffect(() => {
    getClient().admin.GetQuiz(id).then((quiz) => {
      setTitle(quiz.title);
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await getClient().admin.UpdateQuiz(id, { title, is_published: isPublished, pass_threshold: passThreshold, one_attempt: oneAttempt, show_answers: showAnswers, questions, token: "" });
  ...
  return (
    <div style={s.container}>
      <div style={s.header}>
        ...
        <h1 style={s.title}>Редактировать квиз</h1>
      </div>
      ...
      <button style={s.saveBtn} onClick={handleSave} disabled={saving}>{saving ? "Сохранение..." : "Сохранить изменения"}</button>
    </div>
  );
}
